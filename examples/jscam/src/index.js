...
deviceOpenButton.addEventListener("click", function (event) {
  ...
  nokhwaCamera = new NokhwaCamera(constraints).catch((err) => {
    console.error(err);
    return;
  });
...
const streamCaptureImageButton = document.getElementById(
  "streamCaptureImageButton"
);
